...
amendment: 'SCP 2026-04-06 — EA9 (ADR-012 Multi-Turn BMAD), E10/E11/E12 closed, sprint reprioritization toward dogfooding'
totalFRs: 115
totalNFRs: 24
---
...
> Added 2026-03-09 — Sprint Change Proposal approved. Inserted before EA3 (Dashboard).
...
**Package principal:** `@cop1/app` (feature `acceptance-test`)

**Dependencies:** EA1 (BMAD pipeline — DONE), EA2-S0b (ADR-009 BmadStatusReader), cursor-claude-connector (Vercel proxy)
...
> Added 2026-03-11 — Sprint Change Proposal approved. Two-component architecture: BMAD module (standalone) + cop1 API REST.

**User Value:** "iamthelaw rules are managed via a BMAD module with a Judge Dredd agent, exposed via REST API, and active rules enforced during sprints."

**Package principal:** `@cop1/sprint-core` (feature `iamthelaw`) + BMAD module

**Dependencies:** E9 (iamthelaw core), EA5 (sidecar sync)

**Stories:**

- **EA7-S1** : Enrich Rule model (level MUST/SHOULD/MAY) — Sprint 15
- **EA7-S2** : RulesApiHandler (API REST /api/rules) — Sprint 15
- **EA7-S3** : Module BMAD — scaffold (module.yaml, README) — Sprint 15
- **EA7-S4** : Module BMAD — Agent Judge Dredd — Sprint 15
- **EA7-S5** : Module BMAD — Workflow retro-to-rules — Sprint 15
- **EA7-S6** : Module BMAD — Workflow export/import — Sprint 15
- **EA7-S7** : Create initial rules R1-R5 + sidecar activation — Sprint 14

**Definition of Done:**
- Rule model supports MUST/SHOULD/MAY levels with enforcement policy
- API REST `/api/rules` exposes CRUD operations, tested without Fastify
- BMAD module scaffold created with config.yaml and module-help.csv
- Judge Dredd agent can evaluate code against rules
- Retro-to-rules workflow generates rule proposals from retrospective output
- Initial rules R1-R5 (from EA1 retro) created and loaded by sidecar

---

### Epic EA8 — Distribution & Dogfooding cop1

> Added 2026-03-16 — Sprint Change Proposal approved. Two-component: BMAD module cop1-method + CLI @cop1/cli (ADR-011).

**User Value:** "cop1 can be installed on any project via BMAD module + CLI, and can develop itself (dogfooding) with worktree isolation and PR as sole integration point."

**Package principal:** `@cop1/app` + BMAD module `cop1-method`

**Dependencies:** EA6 (acceptance tests validate pipeline), EA1 (BMAD pipeline)

**Stories:**

- **EA8-S1** : Pre-flight checks (BMAD, cop1-method, claude CLI) — Sprint 14
- **EA8-S2** : Worktree → PR creation flow (gh CLI) — Sprint 14 (depends: EA6)
- **EA8-S3** : Module BMAD cop1-method scaffold (config.yaml, module-help.csv) — Sprint 14
- **EA8-S4** : Graceful shutdown + checkpoint (DaemonService SIGTERM) — Sprint 15
...
**Definition of Done:**
- Pre-flight checks validate BMAD, cop1-method module, and claude CLI at startup
- After sprint completion, GitHub PR created from worktree branch via `gh` CLI
- BMAD module cop1-method contains config.yaml and module-help.csv (zero TypeScript)
- DaemonService calls CheckpointService.save() on SIGTERM/SIGINT before exit
...
### Epic EA9 — Multi-Turn BMAD Interaction (ADR-012)

> Added 2026-04-06 — Sprint Change Proposal approved. BLOCKING for automated sprints: replaces broken single-shot `claude -p` with Agent SDK multi-turn sessions.

**User Value:** "cop1 can execute BMAD workflows autonomously — multi-turn conversations with intelligent responses to workflow questions, just like a human developer would."

**Technical approach:**
- New hexagonal port `BMADSessionPort` in `@cop1/sprint-core` (complements existing `BMADCommandPort`)
- Adapter `AgentSdkSessionAdapter` using `@anthropic-ai/claude-agent-sdk` with `settingSources: ["project"]` for BMAD skill access
- `SupervisorService` orchestrates responses: deterministic lookup → LLM supervisor → developer escalation
- `BMADSessionStep` replaces `BMADDevStoryStep` and `BMADReviewStep` as a configurable WorkflowStep
- `ClaudeResumeSessionAdapter` available as fallback via `claude --resume`

**Package principal:** `@cop1/sprint-core` (feature `bmad-orchestration`)

**Dependencies:** EA1 (BMADCommandPort pattern, WorkflowStep interface)

**Stories:**

- **EA9-S1** : BMADSessionPort + AgentSdkSessionAdapter — Port interface + adapter with `startSession()` / `continueSession()` via Agent SDK. Config `settingSources`, `canUseTool` callback for `AskUserQuestion`. Unit tests with mock SDK. (Sprint 11, Medium)
- **EA9-S2** : SupervisorLLMPort + AgentSdkSupervisorAdapter — Port + adapter for the supervisor. Prompt engineering of supervisor prompt. Tests with simulated questions. (Sprint 11, Medium)
- **EA9-S3** : SupervisorService — Orchestrates responses: deterministic → LLM → escalation. Integrates session logging. Depends: EA9-S1, EA9-S2. (Sprint 11, Medium)
- **EA9-S4** : BMADSessionStep — New WorkflowStep replacing BMADDevStoryStep/BMADReviewStep. Integrates BMADSessionPort + SupervisorService. Depends: EA9-S3. (Sprint 11, Medium)
- **EA9-S5** : PipelineStepFactory migration — Replace BMADCommandStep instantiation with BMADSessionSteps. Composition root wiring. Integration test E2E. Depends: EA9-S4. (Sprint 11, Small)
- **EA9-S6** : ClaudeResumeSessionAdapter (fallback) — Alternative adapter via `claude --resume` for cases where the SDK doesn't work. Manual workflow loading as prompts. Depends: EA9-S1 (interface). (Sprint 11, Small)
...
### Sprint 10 — Budget Enforcement + ADR-009 Refactoring (SCP 2026-03-09)
- EA2-S0 (Update epic-ea1 status to done) — **SCP 2026-03-09**
- EA2-S0b (ADR-009 BmadStatusReader refactoring) — **SCP 2026-03-09, BLOCKER**
- EA2-S0c (Consultation iamthelaw integration) — **SCP 2026-03-09**
- EA2-S3, EA2-S4, EA2-S5 (Alerts + pre-call check + CLI)
- EA1-S8 (Integration test) — **DONE** (Sprint 10)

### Sprint 11 — Multi-Turn BMAD Interaction (SCP 2026-04-06, BLOCKING)
- EA9-S1 (BMADSessionPort + AgentSdkSessionAdapter)
- EA9-S2 (SupervisorLLMPort + AgentSdkSupervisorAdapter) — parallel with S1
- EA9-S3 (SupervisorService) — after S1+S2
- EA9-S4 (BMADSessionStep) — after S3
- EA9-S5 (PipelineStepFactory migration) — after S4
...
- EA8-S2 (Worktree → PR creation flow) — depends on EA6
...
### Future
- EA2-S6 (Claude usage API adapter) — not critical V1
...
### Epic EA12 — V1.1 Hardening & Pivots

> **Added 2026-04-15 — Sprint 13 Planning.**
> First post-V1-light sprint. Integrates adversarial review pivots (A5 playbook scrum-directives, A6 status discipline), closes the recovery E2E test gap (A2), delivers real `commit_anchor` + D4 session log aggregation, hardens runtime (A1 budget, A4 reentrance), and audits aspirational features (A7).
> Source: SCP 2026-04-15 (adversarial review — 10 arbitrages, 8 validated, 2 pivots).

**User Value (V1.1):** "cop1 can run a full epic unassisted with real session recovery, real commit anchoring, and simplified configuration — no BMAD command enumeration to maintain, no file-level coupling to worry about."

**Technical approach:**
- **commit_anchor real** (EA12-S1): replace the V1-light stub in `toolCatalog.ts:122-128` with real `git commit` invocation. Conventional format, Co-Authored-By trailer, fail-safe, no auto-push.
- **Recovery E2E** (EA12-S2): test Strategy A (`resume: session_id`) with real `AgentSdkSessionAdapter`. Closes the gap left by EA10-S9 Plan B.
- **Playbook pivot A5** (EA12-S3): remove `commands:` and `allowed_commands` from `PlaybookSchemaV2`. Playbook = scrum directives only. Supervisor discovers BMAD commands autonomously.
- **Status discipline A6** (EA12-S4): delete `BmadStatusReader`. Zero file-level coupling to BMAD state. Status queries via `/bmad-bmm-sprint-status` workflow.
- **Runtime hardening** (EA12-S5): single `max_tokens_per_night` budget cap (A1). Configurable reentrance guard with structured error return (A4).
- **Track 2 extended + D4** (EA12-S6): horodated commands, return codes, decision method, commit SHA anchoring in Track 2 exchange history. Depends on EA12-S1.
- **Aspirational audit** (EA12-S7): delete `kpis-dashboard/`, wire or delete `burndown/` and `velocity-projector/`.

**Package principal:** `@cop1/sprint-core` (toolCatalog, playbook, status) + `@cop1/observability` (Track 2, transcript) + `@cop1/app` (E2E tests)

**Dependencies:** EA10 (done), EA11 (done), EA9 (done)

**Stories:**

- **EA12-S1** — commit_anchor real implementation. Replace V1-light stub with real git commit. Conventional format, Co-Authored-By, fail-safe, no auto-push. Returns real SHA. (Sprint 13, Medium) **Blocks EA12-S6.**
- **EA12-S2** — Recovery E2E test with real AgentSdkSessionAdapter. Test Strategy A (SDK resume:session_id) end-to-end. Crash recovery scenario. Migrates EA10-S9 Plan B sentinel. (Sprint 13, Large) **Blocks dogfooding sign-off.**
- **EA12-S3** — Playbook pivot (A5). Remove commands/allowed_commands from PlaybookSchemaV2. Relax PipelineStepFactory hardcodes. Rewrite supervisor-playbook.md to scrum directives only. (Sprint 13, Medium)
- **EA12-S4** — Status discipline (A6). Delete BmadStatusReader. Wire status queries through invoke_bmad_command → /bmad-bmm-sprint-status. Implement .cop1/orchestrator-state.yaml. Enforce grep invariant. (Sprint 13, Medium)
- **EA12-S5** — Runtime hardening (A1+A4). Single max_tokens_per_night budget cap. Configurable reentrance depth. Structured error return. Track 3 reentrance.cap_hit event. (Sprint 13, Medium)
- **EA12-S6** — Track 2 extended format + session log aggregation (D4). Horodated commands, return codes, decision method, commit SHA anchoring. Story↔session wiring. Depends: EA12-S1. (Sprint 13, Medium)
- **EA12-S7** — Aspirational features audit (A7). Delete kpis-dashboard. Wire or delete burndown + velocity-projector. (Sprint 13, Small)

**Definition of Done (Sprint 13):**
- Recovery E2E test passes with real `AgentSdkSessionAdapter` (not `InMemorySessionAdapter`)
- `commit_anchor` produces real git commits with conventional format and returns real SHAs
- Playbook contains zero BMAD command enumeration — `PlaybookSchemaV2` has no `commands:` field
- `grep -r "sprint-status.yaml" packages/**/src/**/*.ts` returns zero results (A6 invariant)
- Track 2 exchange history includes horodated commands, decision method, and commit SHAs
- `pnpm build && pnpm test` green after all deletions (kpis-dashboard, BmadStatusReader, etc.)

**Deferred to Sprint 14:**
- Layer separation `*Core.ts` extraction (A3) — nice-to-have, non-blocking
- EA8 Distribution stories (EA8-S1, S2, S3)
- Advanced auto-decision policies (original EA10-S3 scope)

**Risks identified upfront:**
1. **EA12-S2 sizing** — real SDK session lifecycle testing may surface unexpected behaviors. Budgeted as Large with 1.5× buffer.
2. **BmadStatusReader consumer migration (EA12-S4)** — number of consumers unknown until grep audit. Could be larger than expected.
3. **B2 open question** — SHA strategy (amend vs 2-pass) for EA12-S6 is unresolved. Will be tranched during EA12-S1 implementation.

**ADR References:** ADR-014 (Amendment 2026-04-15), ADR-013

---

### Epic EA13 — Orchestrator Wiring & Build Fix

> **Added 2026-04-16 — Sprint 14 Planning.**
> Data-driven epic: every story maps to a gap observed during the 2026-04-16 empirical shakedown (`real-run-report-2026-04-16.md`). Source: SCP 2026-04-16. All gaps are wiring gaps (EA9/EA10/EA11/EA12 built the organs — the CLI never routes blood through them). This sprint flips the simulator into a real sprint engine and unblocks `pnpm build` from clean clone.

**User Value (V1.1):** "Running `cop1 orchestrator run --epic <id>` actually runs a BMAD sprint: real commands, real commits, real session history, on a cleanly-built binary."

**Technical approach:**
- **Build unblock** (EA13-S1): clear the ~20 TS errors enumerated in the real-run report. Most are strict-null guards in test fixtures and in `SessionTranscriptGenerator.ts` / `OrchestratorService.ts` / `SupervisorPlaybookLoader.ts`. One structural miss: `ApprovalResolver` export in `@cop1/sprint-core`.
- **Real runner wiring** (EA13-S2): replace `stubRunner` default in `packages/app/src/cli/commands/orchestrator.ts` with a real `BMADCommandRunner` that delegates to `BMADSessionPort` / `SupervisorService`. Stub moves behind an explicit `--runner stub` dev flag.
- **Validation re-run** (EA13-S3): replay the EX1 cobaye procedure from the real-run report. Codify observables as integration test: `.cop1/history/` populated, real git commit with Co-Authored-By trailer, updated story body, real (non-stub) JSONL decisions.
- **Story body update** (EA13-S4): after each phase transition, orchestrator mirrors the status into `<STORY>.md` body. Closes EA12 retro action item #4.

**Package principal:** `@cop1/sprint-core` (exports + strict-null) + `@cop1/app` (CLI wiring, integration test, story mirror hook)

**Dependencies:** EA9 (done), EA10 (done), EA11 (done), EA12 (done)

**Stories:**

- **EA13-S1** — Fix `pnpm build` from clean clone. Restore `ApprovalResolver` export, add strict-null guards in 8 files, resolve duplicate `SupervisorContext` export. (Sprint 14, Small) **Blocks EA13-S2.**
- **EA13-S2** — Real `BMADCommandRunner` default. Replace `stubRunner` default with adapter-driven runner via `BMADSessionPort`. Keep stub behind `--runner stub` flag. (Sprint 14, Medium) **Depends on EA13-S1. Blocks EA13-S3.**
- **EA13-S3** — Validation re-run + integration test. Replay EX1 cobaye and codify observables (track1/2/3, real commit, updated story body, real decisions) as a passing integration test. (Sprint 14, Small) **Depends on EA13-S1, EA13-S2.**
- **EA13-S4** — Story body auto-update on status transition. Orchestrator rewrites `## Status: <value>` in `<STORY>.md` after each phase transition (idempotent). Unit-tested on temp story file. (Sprint 14, Small) **Parallel to S2.**

**Definition of Done (Sprint 14):**
- `pnpm build && pnpm test && pnpm lint` green from cold clone.
- `cop1 orchestrator run --epic EX1 --project-root <sandbox>` produces real BMAD invocations, `.cop1/history/EX1-S1/` populated, ≥1 real git commit with Co-Authored-By trailer, updated `EX1-S1.md` body.
- Stub runner still callable via explicit `--runner stub` — dev/test path preserved.
- Integration test `orchestrator-real-run.test.ts` asserts observables.
...
**Risks identified upfront:**
1. **EA13-S2 boundary crossing** — real `BMADSessionPort` lifecycle may surface latent bugs. Mitigation: keep stub accessible, add observable assertions before marking S2 done.
2. **Build flakiness after S1** — new strict-null guards may mask rather than fix bugs. Mitigation: prefer type narrowing over `!` non-null assertions wherever the code is production (not test) path.
3. **Story body update races** (S4) — if the orchestrator and a human edit the story body concurrently, status may clobber user content. Mitigation: regex target only `^## Status:` line; leave everything else untouched.

**ADR References:** none new. Relies on existing ADR-013 (Orchestrator vs SprintRunner) and ADR-014 (Supervisor Tool Interface).

---

### Epic EA14 — Real-Run Closure & Hygiene

> **Added 2026-04-18 — Sprint 15 Planning.**
> Data-driven epic: every story maps to a gap observed during the post-EA13 empirical shakedown (`real-run-report-2026-04-16.md`) or to a hygiene carry-over from the EA13 retrospective (section 6 action items). Source: SCP 2026-04-18. Three wiring gaps prevent a genuine end-to-end run (BMAD bundle absent, ExchangeHistoryWriter dead code, commit_anchor not prompted); three hygiene items erode baseline quality (SupervisorContext name clash, 9 test failures, 13 lint errors). This sprint closes the remaining 50% of the empirical shakedown gaps.

**User Value (V1.1):** "Running `cop1 orchestrator run --epic <id>` on a project with BMAD installed produces real 3-track history files, real git commits with Co-Authored-By trailer, and exits fast with a clear error when BMAD is absent. The test suite is green and lint passes from cold clone."

**Technical approach:**
- **BMAD bundle detection** (EA14-S1): pre-flight guard in the orchestrator startup path — check that `_bmad/` exists in `projectRoot` and fail fast with a clear error if absent. Prevents silent fiction runs against a project without BMAD.
- **ExchangeHistoryWriter wiring** (EA14-S2): connect the 3-track history writer (EA11-S8 / EA12-S6) into the session adapter lifecycle so Track 1 (raw SDK exchanges), Track 2 (markdown), and Track 3 (structured events) are emitted during real runs. Without this, the entire history subsystem is dead code.
- **Supervisor prompt commit_anchor guidance** (EA14-S3): audit and fix the supervisor system prompt to include explicit instruction for invoking `commit_anchor` after dev-story completion. Verify tool registration in the MCP catalog the supervisor receives.
- **SupervisorContext name clash** (EA14-S4): rename the per-question `SupervisorContext` type (in `SupervisorLLMPort.ts`) to `SupervisorQuestionContext`, update all import sites, remove the EA13 alias workaround.
- **Pre-existing test failures** (EA14-S5): triage 9 failures in `SprintRunner.test.ts` / `bmad-pipeline-e2e.test.ts` — delete deprecated-path tests or fix broken fixtures. Zero test failures target.
- **Baseline lint hygiene** (EA14-S6): fix 13 pre-existing lint errors against the existing configuration. No new lint rules or suppressions.

**Package principal:** `@cop1/sprint-core` (history writer, supervisor prompt, type rename, tests) + `@cop1/app` (pre-flight guard, lint fixes)

**Dependencies:** EA13 (done)

**Stories:**

- **EA14-S1** — BMAD bundle detection pre-flight guard. Check `_bmad/` in `projectRoot` before starting the sprint loop; exit 1 with clear error if absent. Optionally accept `--bmad-root` override. Unit test covers the guard. (Sprint 15, Small) **Blocks EA14-S3.**
- **EA14-S2** — Wire `ExchangeHistoryWriter` into orchestrator session flow. Feed raw exchange data to Track 1 after each round-trip; invoke Track 2 and Track 3 writers after session completion. Integration test asserts `.cop1/history/<storyId>/track{1,2,3}/` populated. (Sprint 15, Medium) **Blocks EA14-S3.**
- **EA14-S3** — Supervisor prompt `commit_anchor` guidance. Audit supervisor system prompt assembly; add explicit post-dev-story instruction to invoke `commit_anchor`. Verify tool is in MCP catalog. Real run produces ≥1 commit with Co-Authored-By trailer. (Sprint 15, Small) **Depends on EA14-S1, EA14-S2.**
- **EA14-S4** — Resolve `SupervisorContext` name clash. Rename per-question type to `SupervisorQuestionContext` in `SupervisorLLMPort.ts`, update all import sites, remove alias export. `grep -r "SupervisorContext" packages/` shows exactly one type definition. (Sprint 15, Small) **Parallel.**
- **EA14-S5** — Investigate + fix 9 pre-existing `SprintRunner` test failures. Triage each: delete deprecated-path tests or fix fixtures. `pnpm test` shows 0 failures (excluding intentional skips with documented rationale). (Sprint 15, Medium) **Parallel.**
- **EA14-S6** — Baseline lint hygiene (13 errors). Fix all errors against existing config; no new suppressions. `pnpm lint` exits 0. (Sprint 15, Small) **Parallel.**

**Definition of Done (Sprint 15):**
- `cop1 orchestrator run --epic EX1 --project-root /tmp/...` on a project with `_bmad/` installed produces `.cop1/history/EX1-S1/track{1,2,3}/` populated and ≥1 real git commit with Co-Authored-By trailer.
- Same command on a project without `_bmad/` exits 1 with a clear error message.
- `pnpm build && pnpm test && pnpm lint` green from cold clone — zero test failures, zero lint errors.
- No duplicate `SupervisorContext` type name in `@cop1/sprint-core`.
- Stub runner still usable via `--runner stub` — existing tests not regressed.
...
**Risks identified upfront:**
1. **EA14-S2 adapter lifecycle bugs** — real BMAD invocation (once S1 ensures `_bmad/` is present) may surface latent bugs in `BMADSessionPort` or `ExchangeHistoryWriter`. Mitigation: S1 fail-fast guard prevents silent fiction; S2 is testable in isolation via a unit test that asserts Track 1/2/3 file creation.
2. **EA14-S3 prompt drift** — supervisor system prompt may be assembled dynamically, making it hard to guarantee `commit_anchor` guidance survives prompt compaction. Mitigation: verifiable by inspecting assembled prompt via debug log or test.
3. **EA14-S5 deprecated test deletion** — removing tests reduces coverage count. Mitigation: each deletion has a documented rationale tied to EA11-S1/S2 deprecation.
...
| Sprint 12 | 17 stories | 70 pts | 600 pts |
| Sprint 13 | 7 stories | ~30 pts | ~630 pts |
| Sprint 14 | 4 stories | ~12 pts | ~642 pts |
| Sprint 15 | 6 stories | ~20 pts | ~662 pts |
| **Subtotal Phase A** | **58 stories** | **~227 pts** | — |

> **Note Sprint 12:** EA10 (9 stories) and EA11 (8 stories) completed concurrently 2026-04-14. V1-light DoD closed.
> **Note Sprint 13:** First V1.1 sprint. EA12 (7 stories) — hardening + pivots from adversarial review.
> **Note Sprint 14:** EA13 (4 stories) — data-driven hardening from the 2026-04-16 empirical shakedown. Build unblock + real runner wiring + story body mirror.
> **Note Sprint 15:** EA14 (6 stories) — real-run closure (3 wiring gaps) + hygiene (name clash, test failures, lint). Closes remaining 50% of empirical shakedown gaps.
...
| Phase A (EA1-EA14) | 58 | ~227 |
| **Total** | **168 stories** | **~662 pts** |
...
**Phase A stories : 58 stories covering BMAD pivot + V1.1 hardening (EA1-EA14)**
